...
const STREAMS = [
  ...
  { name: 'Sky News',    url: 'https://linear219.akamaized.net/channel(skynewshd)/index.m3u8', color: '#5dcaa5', abbr: 'SKY' },
...
export default function CinemaPanel() {
  ...
  useEffect(() => { setError(false) }, [active])

  return (
    <div className="panel">
      <div className="panel-header">
        <span className="panel-title">Cinema Feeds</span>
        <span className="panel-badge">{STREAMS.length} LIVE</span>
      </div>
      ...
      <div style={{ padding: '5px 5px 0', flexShrink: 0 }}>
        <div style={{
          ...
          <div style={{ position: 'absolute', inset: 0 }}>
            {!error ? (
              <HLSPlayer key={active} url={STREAMS[active].url} />
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 8 }}>
                <div style={{ fontSize: 9, color: '#3a4a58' }}>Stream unavailable</div>
                <a href={STREAMS[active].url} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: '#378add' }}>Open direct link</a>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3, padding: 5, flex: 1 }}>
        {STREAMS.map((s, i) => (
          <div key={i} onClick={() => setActive(i)} style={{
            ...
            border: '1px solid ' + (active === i ? s.color : '#141a22'),
            ...
            {active === i && <div style={{ marginLeft: 'auto', fontSize: 7, color: '#e24b4a' }}>LIVE</div>}
          </div>
        ))}
      </div>
